...
class TeacherWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        logger.info("Окно преподавателя успешно инициализировано")
    ...
    def setup_toolbar(self):
        ...
        for title, icon, handler in actions:
            icon_path = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "resources", "icons", icon
            )
    ...
    def setup_buttons(self, layout):
        ...
        for format_name, display_text in export_formats.items():
            ...
            action.triggered.connect(
                lambda checked, f=format_name: self.handle_export(f)
            )
    ...
    def show_stats(self):
        ...
        try:
            ...
            logger.debug("Открыто окно статистики")
        except Exception as e:
            logger.error("Ошибка при открытии окна статистики", exc_info=True)
            QMessageBox.critical(
                self, "Ошибка", f"Не удалось открыть статистику: {str(e)}"
            )
    ...
    def show_filters(self):
        ...
        QMessageBox.information(
            self, "Фильтры", "Фильтры будут доступны в следующей версии"
        )
    ...
    def archive_selected(self):
        ...
        selected_rows = set(item.row() for item in self.results_table.selectedItems())
        if not selected_rows:
            QMessageBox.warning(self, "Предупреждение", "Выберите строки для архивации")
            return
        ...
        QMessageBox.information(
            self, "Успех", f"Архивировано строк: {len(data_to_archive)}"
        )
    ...
    def handle_logout(self):
        ...
        try:
            ...
            logger.info("Выполнен выход из аккаунта преподавателя")
        except Exception as e:
    ...
    def handle_export(self, format_name):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Ошибка", f"Ошибка при экспорте данных: {str(e)}", QMessageBox.Ok
            )
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            if self.stats_widget:
                self.stats_widget.close()
            event.accept()
            logger.info("Окно преподавателя закрыто")
        except Exception:
            logger.error("Ошибка при закрытии окна", exc_info=True)
            event.ignore()
