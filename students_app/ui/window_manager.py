        ...
        try:
            ...
            if self.current_window is not None:
                try:
                    ...
                    logger.warning("Текущее окно уже было закрыто")
            ...
            logger.info(f"Успешно показано окно класса {new_window.__class__.__name__}")

        except Exception as e:
            logger.exception(f"Ошибка при попытке показать окно: {e}")
            raise
